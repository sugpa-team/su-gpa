...
class SUcheduleCourseScraper:
    ...
    def write_json_file(self, courses: List[Dict], instructors: List[str], places: List[str]):
        """
        Write json file. Output path: frontend/public/data/schedule_data/{term}.min.json
        relative to the repo root. This script lives at scripts/scrape_suchedule/,
        so the repo root is two directories up.
        """
        import os
        script_dir = os.path.dirname(os.path.abspath(__file__))
        repo_root = os.path.abspath(os.path.join(script_dir, os.pardir, os.pardir))
        target_dir = os.path.join(repo_root, "frontend", "public", "data", "schedule_data")
